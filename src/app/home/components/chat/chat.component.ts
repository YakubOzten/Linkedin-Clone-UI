...
export class ChatComponent  {
  ...
  selectedConversetionIndex:number=0;
  ...
  ionViewDidEnter() {
    ...
    //TODO:refector - unsubscribe
   this.messagesSubscription = this.chatService.getConversationMessages().subscribe((messsages:Message[])=>{
       
    messsages.forEach((message:Message)=>{
  ...
  openConversation(friend:User,index:number):void{
    this.selectedConversetionIndex=index;
  ...
  ionViewDidLeave():void {
    this.chatService.leaveConversation();
    this.selectedConversetionIndex = 0; 
    this.conversations = [];
